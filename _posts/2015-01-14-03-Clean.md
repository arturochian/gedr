...
##Merging Data
Another very common clean-up/data managment task is to be able to merge data into a single data frame.  We are going to talk about several different ways to do this.  First, is adding a new column or existing vector to a data frame with `cbind()`.  Think of it as binding a column to the data frame.  We do it this way:
...
This is handy to know, but you need to be very careful with it.  First, the data in the vector you are adding needs to be in the same order as the observations in the data frame.  Also it needs to have the same number of rows (or divisible by that number of rows).  Given this limitation, their might be better ways to add a single, newly created vector to a data frame.  That is where `transform()` comes in handy.




The next thing we might want to do is add some new rows to a data frame.  This is very handy as you might have data collected and entered at one time, and then additional observations made later that need to be added.  We can do this with `rbind()`
...
{% highlight text %}
##    a b     c             d
## 1  1 a  TRUE  0.1805797326
## 2  2 b  TRUE  0.1201824602
## 3  3 c FALSE  0.6053891411
## 4 10 x FALSE -0.6449256397
## 5 11 y FALSE -0.5852459160
## 6 12 z FALSE  0.0002111774
{% endhighlight %}
